...
**[⬇️ Cài đặt](#-cài-đặt)**  ·  **[📖 Tài liệu đầy đủ](https://sonnam0904.github.io/telebit/)**  ·  **[⚙️ Cấu hình](#-tuỳ-chọn-cấu-hình)**  ·  **[🩺 Sự cố](#-xử-lý-sự-cố)**  ·  **[🐞 Báo lỗi](https://github.com/sonnam0904/telebit/issues/new)**
...
| **Debian / Ubuntu bản khác**     | [Gói](#install-deb) `.deb` [tải tay](#install-deb)                   |
| **Fedora / CentOS**              | [Gói](#install-rpm) `.rpm`                                           |
...
**Cách 2 — Gói** `.deb` **tải tay** (Debian / Ubuntu)
...
**Cách 4 — Build từ source** (mọi distro: Arch, Debian, Fedora, …)
...
**Gói đã làm sẵn bước này** — nó cài `/usr/lib/environment.d/60-telebit-fcitx5.conf`
(bản `install.sh --user` đặt ở `~/.config/environment.d/`) đặt `GTK_IM_MODULE=fcitx`,
`QT_IM_MODULE=fcitx`, `XMODIFIERS=@im=fcitx`. systemd chỉ đọc file này lúc khởi tạo user
session, nên phải **logout / login** (hoặc reboot). Kiểm tra sau khi login lại:
...
Ứng dụng **Flatpak cũng nhận được luôn**, không cần `flatpak override`: flatpak kế thừa
môi trường của tiến trình khởi chạy nó và không lọc bỏ ba biến này.

Nếu biến vẫn trống sau khi login lại

Session không chạy qua systemd user manager, hoặc `/etc/environment` đang đè lên. Dùng
cách truyền thống:
...
hoặc thêm vào `~/.profile` / `~/.xprofile`:
...
| **ToggleVietnameseKey**    | Ctrl+Shift+Z | Tạm bật/tắt gõ tiếng Việt — tiện khi cần gõ một đoạn tiếng Anh dài.                                                                                                                                                                                 |
...
Nhấn Ctrl+Shift+Space ngay trong lúc gõ để mở ô yêu cầu, viết prompt — dán thêm ngữ cảnh bằng Ctrl+V nếu muốn — rồi Enter. Văn bản AI sinh ra được chèn thẳng vào ứng dụng. Dùng tốt cho: soạn/trả lời tin nhắn, dịch, tóm tắt, sửa câu.
...
| Tiếng Anh bị biến dạng                      | Bật **SpellCheckRestore**, hoặc Ctrl+Shift+Z để tắt tạm                               |
...
**Bấm để xem lệnh gỡ theo từng cách đã cài**
...
**Build từ source, chạy test, cấu trúc code**
